#!/usr/bin/env python3
"""
Short Drama Tracker — Main entry point
...
def main():
    ...
    # Step 1: Crawl
    if crawl_only:
        print("📺 Short Drama Tracker — Starting crawl")
        print()
    ...
    if export_only:
        ...
        if data:
            total_dramas = sum(len(p.get('current', [])) for p in data['platforms'].values())
            print(f"  ✅ Exported {total_dramas} drama entries")
            print(f"  📁 {EXPORT_PATH}")
        else:
